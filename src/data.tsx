...
export function playersForMatch(
  ...
  const red = roster.Red.slice(0, playersPerTeam).map((p: any, rosterIndex: number) => ({
    ...p,
    rosterIndex,
    team: "red",
  }));

  const blue = roster.Blue.slice(0, playersPerTeam).map((p: any, rosterIndex: number) => ({
    ...p,
    rosterIndex,
    team: "blue",
  }));
...
export function homeTotals(dayConfigs: any[], days: number, players: number, states: any) {
  const total: any = {
    ...
    if (bare) {
      return (
        <img
          src={logoSrc}
          alt=""
          className={cx("aspect-square shrink-0 rounded-full object-cover", size)}
        />
      );
    }
