...
export default function PlayerScorecard({
  ...
  async function shareCard() {
    ...
    if (navigator.share) {
      await navigator.share({ title: "DUEL Scorecard", text });
    }
  }
      ...
      <div className="relative h-[94vh] w-full max-w-[390px] overflow-hidden rounded-[32px] border border-white/10 bg-black shadow-2xl">
        ...
        <div className="relative z-10 flex h-full flex-col px-3 pb-[72px] pt-[102px]">
          ...
          <div className="mt-5 overflow-hidden rounded-[18px] border border-white/18 bg-black/90 shadow-[0_24px_55px_rgba(0,0,0,0.78)] backdrop-blur-xl">
            ...
            >
              <Summary label="Par" value={played.length ? parPlayed : "-"} />
...
function NineScoreTable({ title, rows, accent, scoreMode }: any) {
        ...
        {rows.map((h: any) => {
          ...
          return (
            <div key={h.hole} className="flex items-center justify-center">
              <ScoreMark gross={value} par={h.par} />
            </div>
          );
        })}
...
function Summary({ label, value, bordered = false }: any) {
  return (
    <div className={bordered ? "border-x border-white/20" : ""}>
      ...
      <div className="mt-0.5 text-[22px] font-black leading-none">{value}</div>
    </div>
  );
}
